...
(async () => {
  ...
  try {
    ...
    const merchant = process.env.VERIFY_MERCHANT === '1';
    ...
    for (const width of [1440, 320]) {
      ...
      await context.addInitScript((merchant) => {
        const token = btoa('{}') + '.' + btoa(JSON.stringify({ sub: 'test-user', role: merchant ? 'PRESTATAIRE' : 'CLIENT', exp: Date.now()/1000+3600 })) + '.test';
        localStorage.setItem('accessToken', token);
        localStorage.setItem('currentUser', JSON.stringify({ id:'test-user', name:'Client test', role: merchant ? 'PRESTATAIRE' : 'CLIENT' }));
      }, merchant);
      ...
      if (merchant) {
        pharmacy.pharmacy.userId = 'test-user'; pharmacy.status = 'EN_ATTENTE_PHARMACIE';
        material.hardwareStore.userId = 'test-user'; material.status = 'EN_ATTENTE_QUINCAILLERIE';
      }
      await context.route('http://localhost:3000/**', async route => {
        const path = new URL(route.request().url()).pathname;
        let data = [];
        if(path.endsWith('/users/me')) data = {id:'test-user',nom:'Client test',role: merchant ? 'PRESTATAIRE' : 'CLIENT'};
        ...
        else if(path.endsWith('/access')) data={isPharmacy:merchant,isHardwareStore:merchant};
        await route.fulfill({json:{success:true,data}});
      });
      for(const route of routes.filter(route => !merchant || ['pharmacy-orders', 'material-orders', 'pharmacy-orders/test', 'material-orders/test'].includes(route))) {
        ...
        const otherFonts = await host.evaluate(el => [...el.querySelectorAll('*')].filter(node => node.getClientRects().length && [...node.childNodes].some(child => child.nodeType === 3 && child.textContent.trim()) && !getComputedStyle(node).fontFamily.includes('Inter')).map(node => ({tag:node.tagName,font:getComputedStyle(node).fontFamily})).slice(0, 5));
        if(otherFonts.length) throw new Error(route+' inconsistent fonts '+JSON.stringify(otherFonts));
        ...
        await page.screenshot({path:path.join(artifacts,(merchant ? 'merchant-' : '')+route.replaceAll('/','-')+'-'+width+'.png')});
