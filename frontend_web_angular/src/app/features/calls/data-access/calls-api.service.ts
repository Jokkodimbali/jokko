...
import type { CallHistoryItem, CallKind } from '../domain/call.models';

@Injectable({ providedIn: 'root' })
export class CallsApiService {
  private readonly http = inject(HttpClient);
  createJoinCredential(conversationId: string, callId: string, kind: CallKind) {
    return this.http
      .post<
        ApiResponse<{ serverUrl: string; token: string }>
      >(`${environment.apiUrl}/calls/conversations/${conversationId}/join-credential`, { callId, kind })
