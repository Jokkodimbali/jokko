...
@Component({
  ...
  standalone: true,
  template: `<div #canvas class="map" role="img" [attr.aria-label]="'Localisation de ' + label"></div>
    <div hidden><div #markerContent class="merchant-marker" [attr.aria-label]="label">
      @if (avatarUrl && failedAvatar() !== avatarUrl) {
        <img [src]="avatarUrl" [alt]="label" referrerpolicy="no-referrer" (error)="failedAvatar.set(avatarUrl)" />
      } @else { <span>{{ initials(label) }}</span> }
    </div></div>
...
export class LocationMapComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  ngAfterViewInit(): void { this.ready = true; void this.render(); }
  ngOnChanges(): void { if (this.ready) void this.render(); }
  ngOnDestroy(): void {
    this.destroyed = true;
  ...
  private async render(): Promise<void> {
    ...
    if (!this.runtime) {
      ...
      this.loading = true;
      try { this.runtime = await this.loader.load(); }
      catch { this.unavailable.set(true); return; }
    ...
    if (!this.map) {
      this.map = new this.runtime.maps.Map(this.canvas.nativeElement, {
        center, zoom: 15, mapId: this.runtime.mapId,
    ...
    if (this.runtime.maps.marker) {
      if (!this.marker) this.marker = new this.runtime.maps.marker.AdvancedMarkerElement({ map: this.map, position: center, title: this.label, content: this.markerContent.nativeElement });
      else { this.marker.position = center; this.marker.title = this.label; this.marker.map = this.map; }
    }
